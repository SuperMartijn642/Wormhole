    ...
    public static void onBlockHighlight(DrawBlockHighlightEvent.HighlightBlock e){
        ...
        if(entity instanceof GeneratorBlockEntity){
            ...
            for(BlockPos pos : ((GeneratorBlockEntity)entity).getChargingPortalBlocks()){
                VoxelShape shape = level.getBlockState(pos).getOcclusionShape(level, pos).move(pos.getX(), pos.getY(), pos.getZ());
                RenderUtils.renderShape(shape, 66 / 255f, 108 / 255f, 245 / 255f, true);
            }
            for(BlockPos pos : ((GeneratorBlockEntity)entity).getChargingEnergyBlocks()){
                VoxelShape shape = level.getBlockState(pos).getOcclusionShape(level, pos).move(pos.getX(), pos.getY(), pos.getZ());
                RenderUtils.renderShape(shape, 242 / 255f, 34 / 255f, 34 / 255f, true);
            }
