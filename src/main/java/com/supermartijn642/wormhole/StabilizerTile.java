...
public class StabilizerTile extends PortalGroupTile implements ITargetCellTile, IEnergyCellTile {
    ...
    public boolean activate(PlayerEntity player){
        if(this.hasGroup()){
            ...
            if(shape == null)
                player.sendMessage(new TranslationTextComponent("wormhole.portal_stabilizer.error").withStyle(TextFormatting.RED));
            else{
                this.level.getCapability(PortalGroupCapability.CAPABILITY).ifPresent(groups -> groups.add(shape));
                player.sendMessage(new TranslationTextComponent("wormhole.portal_stabilizer.success").withStyle(TextFormatting.YELLOW));
            }
        }
        return true;
    }
