...
public class WormholeConfig {
    ...
    private WormholeConfig(ForgeConfigSpec.Builder builder){
        ...
        this.requireDimensionalCore = builder.worldRestart().comment("Does a portal require a dimensional core for interdimensional travel?").define("requireDimensionalCore", true);
        builder.pop();
        ...
        this.distancePowerDrain = builder.worldRestart().comment("How much energy is drained when an entity is teleported multiplied by the distance?").defineInRange("distancePowerDrain", 1, 0, 1000000d);
        this.dimensionPowerDrain = builder.worldRestart().comment("How much energy is drained when an entity is teleported to another dimension?").defineInRange("dimensionPowerDrain", 500, 0, 1000000);
        builder.pop();
        builder.push("Energy Cells");
        this.stabilizerEnergyCapacity = builder.worldRestart().comment("The amount of energy the portal stabilizer can store.").defineInRange("basicEnergyCellCapacity", 1000, 1, 1000000);
        this.basicEnergyCellCapacity = builder.worldRestart().comment("The amount of energy the basic energy cell can store.").defineInRange("basicEnergyCellCapacity", 1000, 1, 1000000);
        ...
        this.basicTargetCellCapacity = builder.worldRestart().comment("The number of targets the basic target cell can store.").defineInRange("basicTargetCellCapacity", 5, 1, 20);
        this.advancedTargetCellCapacity = builder.worldRestart().comment("The number of targets the advanced target cell can store.").defineInRange("advancedTargetCellCapacity", 10, 1, 20);
        builder.pop();
    }
