...
public class PortalOverviewScreen extends PortalGroupScreen {
    ...
    protected void render(int mouseX, int mouseY){
        ...
        this.font.drawString(new TranslationTextComponent("wormhole.portal.gui.status").getFormattedText(), 190, 49, 4210752);
        this.drawStringRightAligned(status.getStatus(), 312, 49);
        this.font.drawString(new TranslationTextComponent("wormhole.portal.gui.idle_cost").getFormattedText(), 190, 61, 4210752);
        this.drawStringRightAligned(new StringTextComponent(EnergyFormat.formatEnergyPerTick(this.getFromPortalGroup(PortalGroup::getIdleEnergyCost, 0))), 312, 61);
        this.font.drawString(new TranslationTextComponent("wormhole.portal.gui.teleport_cost").getFormattedText(), 190, 73, 4210752);
        this.drawStringRightAligned(new StringTextComponent(this.getFromPortalGroup(PortalGroup::getActiveTarget, null) == null ? "--" : EnergyFormat.formatEnergy(this.getFromPortalGroup(PortalGroup::getTeleportEnergyCost, 0))), 312, 73);
        // target number
    ...
    private enum PortalStatus {
        ...
        public ITextComponent getStatus(){
            return new StringTextComponent(this.status).applyTextStyle(this.color);
        }
    }
}
