 ...
 */
public class PortalTargetNameField extends Widget implements IRenderable, IGuiEventListener, ITickableWidget {
    ...
    public PortalTargetNameField(PortalGroupScreen screen, Supplier<Integer> targetIndex, int x, int y){
        super(x, y, 59, 10, I18n.format("wormhole.target.name"));
        this.screen = screen;
        this.targetIndex = targetIndex;

        PortalTarget target = screen.getPortalGroup().getTarget(targetIndex.get());
        this.text = target == null ? "" : target.name;
    ...
    private void onTextChanged(String text){
        Wormhole.CHANNEL.sendToServer(new PortalNameTargetPacket(this.screen.getPortalGroup(), this.targetIndex.get(), text));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button){
        if(!this.visible){
            ...
            if(this.isFocused() && flag && button == 0){
                ...
                FontRenderer font = this.screen.getMinecraft().fontRenderer;
                String s = font.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getAdjustedWidth());
                this.setCursorPosition(font.trimStringToWidth(s, i).length() + this.lineScrollOffset);
                return true;
            }else{
                return false;
            }
    ...
    public void renderButton(int mouseX, int mouseY, float partialTicks){
        if(this.visible){
            ...
            fill(this.x - 1, this.y - 1, this.x + this.width + 1, this.y + this.height + 1, i);
            ...
            if(!s.isEmpty() && flag && j < s.length()){
                font.drawStringWithShadow(s.substring(j), (float)j1, (float)i1, i2);
            }
