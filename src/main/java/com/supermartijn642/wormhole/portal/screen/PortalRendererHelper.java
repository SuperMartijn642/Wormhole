...
public class PortalRendererHelper {
    ...
    private static void renderBlock(Level level, BlockPos pos, PoseStack poseStack, MultiBufferSource bufferSource, boolean valid){
        ...
        int tint = ClientUtils.getMinecraft().getBlockColors().getColor(state, level, pos);
        float red = ARGB.redFloat(tint), green = ARGB.greenFloat(tint), blue = ARGB.blueFloat(tint);
        if(!valid){
            red *= 0.5f;
            green *= 0.5f;
            blue *= 0.8f;
        }
        ModelBlockRenderer.renderModel(poseStack.last(), bufferSource.getBuffer(renderType), model, red, green, blue, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY);
