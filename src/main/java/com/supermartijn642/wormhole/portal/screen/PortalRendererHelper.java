...
public class PortalRendererHelper {
    ...
    public static void updateState(RenderState renderState, PortalShape shape){
        ...
        for(BlockPos pos : frame){
            ...
            List<BlockStateModelPart> parts = blockRenderState.setupModel(IDENTITY_MATRIX, model.hasMaterialFlag(level, pos, state, BakedQuad.FLAG_TRANSLUCENT));
            RANDOM_SOURCE.setSeed(state.getSeed(pos));
            model.collectParts(level, pos, state, RANDOM_SOURCE, parts);
