...
public class TargetDeviceScreen extends WormholeScreen {
    ...
    protected void addWidgets(){
        ...
        // current location
        this.currentTextField = this.addWidget(new TextFieldWidget(this.font, 8, 0, 59, 10, new StringTextComponent("wormhole.target_device.gui.target_name")));
    ...
    protected void render(MatrixStack matrixStack, int mouseX, int mouseY){
        ...
        this.drawTop(matrixStack, targets, this.sizeX(), height);
        matrixStack.translate(0, height + 1, 0);
        this.drawBottom(matrixStack, this.sizeX(), BASE_HEIGHT + HEIGHT_PER_TARGET);
    }

    private void drawTop(MatrixStack matrixStack, List<PortalTarget> targets, float width, float height){
        this.drawBackground(matrixStack, 0, 0, width, height);
        this.font.func_243248_b(matrixStack, this.title, 8, 7, 4210752);
    }

    private void drawBottom(MatrixStack matrixStack, float width, float height){
        this.drawBackground(matrixStack, 0, 0, width, height);
        this.font.func_243248_b(matrixStack, new TranslationTextComponent("wormhole.target_device.gui.current_location"), 8, 7, 4210752);
    }
