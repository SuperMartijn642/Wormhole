...
public class TargetDeviceScreen extends ItemBaseWidget {
    ...
    @Override
    protected void renderBackground(MatrixStack poseStack, int mouseX, int mouseY, ItemStack object){
    ...
    @Override
    protected void renderForeground(MatrixStack poseStack, int mouseX, int mouseY, ItemStack object){
        ...
        if(this.selectedTarget >= 0){
            ...
            RegistryKey<World> dimension = ClientUtils.getWorld().dimension();
    ...
    private void renderTargetInfo(MatrixStack poseStack, String name, BlockPos pos, RegistryKey<World> dimension, ITextComponent dimensionName, float yaw){
        ScreenUtils.drawCenteredString(poseStack, name, 162, 31, Integer.MAX_VALUE);

        ScreenUtils.bindTexture(SEPARATOR);
        ScreenUtils.drawTexture(poseStack, 124, 41, 77, 1);

        // location
        GlStateManager._enableAlphaTest();
        ScreenUtils.bindTexture(LOCATION_ICON);
        ScreenUtils.drawTexture(poseStack, 121, 47, 9, 9);
        ...
        if(block == null){
            ...
            ScreenUtils.drawTexture(poseStack, 121, 59, 9, 9);
        }else{
        ...
        ScreenUtils.drawTexture(poseStack, 119, 69, 13, 13);
    ...
    @Override
    public void renderTooltips(MatrixStack poseStack, int mouseX, int mouseY, ItemStack object){
        // location
