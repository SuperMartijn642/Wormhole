...
public class StabilizerBlockEntity extends PortalGroupBlockEntity implements ITargetCellEntity, IEnergyCellEntity {
    ...
    public boolean activate(EntityPlayer player){
        if(this.hasGroup()){
            ...
            if(shape == null)
                player.sendStatusMessage(TextComponents.translation("wormhole.portal_stabilizer.error").color(TextFormatting.RED).get(), true);
            else{
                PortalGroupCapability groups = this.world.getCapability(PortalGroupCapability.CAPABILITY, null);
                if(groups != null)
                    groups.add(shape);
                player.sendStatusMessage(TextComponents.translation("wormhole.portal_stabilizer.success").color(TextFormatting.YELLOW).get(), true);
            }
        }
        return true;
    }
